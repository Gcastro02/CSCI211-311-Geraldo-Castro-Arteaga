// CSCI 311
// Lab 6 - Hash Table cpp (unsigned-accumulation variant)
// Author: autogenerated copy for testing
...
int HashTable::hash(std::string s){
  if (size <= 0) return -1;
  unsigned int h = 0u;
  unsigned int powp = 1u;
  unsigned int up = static_cast<unsigned int>(p);
  for (char c : s){
    unsigned int val = static_cast<unsigned char>(c);
    h += val * powp; // allow wrap-around
    powp *= up;      // allow wrap-around
  }
  return static_cast<int>(h % static_cast<unsigned int>(size));
}
